...
import { updatePedidoAction } from '@/app/lib/actions/pedidoActions';
import { useActionState, useState, useEffect } from 'react';
import { UserGroupIcon, PlusIcon, TrashIcon } from '@heroicons/react/24/outline';
import type { Cliente, Pedido } from '@/app/lib/api/componenteA';
...
export default function EditForm({
  ...
  const updatePedidoWithId = updatePedidoAction.bind(null, pedido.id);
  const [state, formAction] = useActionState(updatePedidoWithId, initialState);

  const [productos, setProductos] = useState(
    pedido.productos.map(p => ({ nombre: p.nombre, precio: p.precio.toString() }))
  );
